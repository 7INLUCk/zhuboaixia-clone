...
type FixedChat = {
  ...
  trigger: string
  response: string
  outfitId: string | null   // 选造型
  actionId: string | null   // 选该造型下的动作
}
        ...
        <div style={{ display: 'flex', gap: 4, overflowX: 'auto' }}>
          {TABS.map(tab => (
            <button key={tab.key} onClick={() => onSwitchPanel?.(tab.key)} style={{
              ...
              fontWeight: tab.key === 'autoChat' ? 600 : 400,
              background: tab.key === 'autoChat' ? C.blueLight : 'transparent',
              color: tab.key === 'autoChat' ? C.blue : C.textSec,
                          ...
                          <div key={chat.id} style={{
                            ...
                            {editingId === chat.id ? (
                              <>
                                ...
                                {/* 造型选择 */}
                                <div style={{ fontSize: 10, color: C.textTert, marginBottom: 4 }}>切换造型（选动作前必须先选造型）</div>
                                  ...
                                  {chat.outfitId && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                      ...
                                      <span style={{ fontSize: 11, color: C.textSec }}>
                                        ...
                                        {!chat.actionId && <span style={{ color: C.textTert }}> · 默认状态</span>}
                                      </span>
                                    </div>
                                  )}
                                </div>
