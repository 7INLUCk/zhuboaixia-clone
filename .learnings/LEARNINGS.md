...
## 2026-04-07 17:15 - 删功能前未确认组件的完整使用场景（Boss 纠正）

### 问题
误删了入场/退场的全部入口（数据+UI+飞书文档），包括应该保留的 Timeline 预览按钮。

### 根因
- 入场/退场有**两套完全独立的 UI**：
  1. Step 3 特殊动作区块（口令触发的动作列表）→ 应该过滤掉
  2. Timeline 左栏预览按钮（🎬入场动画 + 🚪退场动画）→ 必须保留
- 只看到 Step 3 里的入口，就认为应该全部删除，没有 grep 全局引用

### 正确做法
- 删功能前：`grep -r "ea_entrance\|ea_exit\|入场\|退场\|isTransition"` 确认所有引用点
- 只过滤 Step 3 的展示（`.filter(ev => !ev.isTransition)`），不动其他地方
- Timeline 预览按钮是用户手动触发的预览工具，和口令触发是独立逻辑

### 防复发
- 删除任何功能前，必须先 grep 全局搜索所有引用点
- 确认每个引用点的使用场景后再决定保留还是删除
- "一个功能出现在多个地方"是常态，不能只看一个文件下结论
