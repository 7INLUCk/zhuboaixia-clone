...
## 🔴 2026-04-07 声控切镜逻辑 V2 方案（待落地）

**存档文件**：`memory/voice-switch-logic-v2.md`

**核心变化**：
- 旧：展示口令 = 进场 + 换装（根据口令中的链接号）
- 新：拆成三种独立口令 → 进场口令 / 退场口令 / 指定链接口令

**三层逻辑**：
1. **进场口令**：匹配「讲解中商品」（运营后台手动切换），播放入场动画
2. **退场口令**：不变，播放退场动画离场
3. **指定链接口令**：硬切到具体链接，有形象在场直接切，无形象正常入场
...
## 🔴 图片/视频分析必须用 qwen-vision skill（2026-04-07 Boss确认）
- **触发场景**：Boss 发图片或视频要求分析时
- **正确方式**：调用 `qwen-vision` skill（`~/.agents/skills/qwen-vision/SKILL.md`）
- **禁止方式**：使用内置 `read` 工具分析图片/视频
- **原因**：内置 read 工具的视觉分析能力不足，qwen-vision 更适合处理多媒体内容
