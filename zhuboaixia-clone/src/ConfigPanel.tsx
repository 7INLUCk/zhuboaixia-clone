...
// 8 Tab 定义
type ConfigTab = 'live' | 'script' | 'reply' | 'autoSpeak' | 'tools' | 'coupons' | 'luckyBag' | 'widget'
const CONFIG_TABS: { key: ConfigTab; label: string }[] = [
  { key: 'live', label: '直播(麦克风)' },
  { key: 'script', label: '话术' },
  { key: 'reply', label: '待回复' },
  { key: 'autoSpeak', label: '自动发言' },
  { key: 'tools', label: '互动工具' },
  { key: 'coupons', label: '发券' },
...
// 智能回复子项
type SmartReplyItem = { id: string; label: string; emoji: string }
const SMART_REPLY_ITEMS: SmartReplyItem[] = [
  { id: 'commentReply', label: '评论回复', emoji: '' },
  { id: 'luckyDraw', label: '福袋/抽奖', emoji: '' },
  { id: 'greet', label: '打招呼', emoji: '' },
  { id: 'thankGift', label: '感谢送礼', emoji: '' },
  { id: 'thankFollow', label: '感谢关注', emoji: '' },
  { id: 'productExplain', label: '商品讲解', emoji: '' },
  { id: 'askCoupon', label: '问优惠券', emoji: '' },
  { id: 'askReview', label: '要好评', emoji: '' },
  { id: 'pickReply', label: '捡宝回复', emoji: '' },
  { id: 'answer', label: '答题', emoji: '' },
]

// 直播 Toggle 项
type LiveToggleItem = { id: string; label: string; emoji: string; expandable?: boolean }
const LIVE_TOGGLE_ITEMS: LiveToggleItem[] = [
  { id: 'greet', label: '打招呼', emoji: '💬' },
  { id: 'thankFollow', label: '感谢关注', emoji: '❤️' },
  { id: 'thankGift', label: '感谢送礼物', emoji: '🎁' },
  { id: 'smartReply', label: '智能回复', emoji: '🤖', expandable: true },
  { id: 'productExplain', label: '直播间商品讲解', emoji: '📦' },
  { id: 'activityReminder', label: '活动提醒', emoji: '⏰' },
  { id: 'welcomeThanks', label: '欢迎感谢', emoji: '👋' },
]
...
export default function ConfigPanel({ onClose }: Props) {
  ...
  // 配置面板 Tab 状态
  const [activeConfigTab, setActiveConfigTab] = useState<ConfigTab>('live')

  // 直播 Tab toggle 状态
  const [liveToggleStates, setLiveToggleStates] = useState<Record<string, boolean>>({
    greet: true,
    thankFollow: true,
    thankGift: true,
    smartReply: false,
    productExplain: false,
    activityReminder: false,
    welcomeThanks: true,
  })

  // 智能回复展开状态
  const [smartReplyExpanded, setSmartReplyExpanded] = useState(false)

  // 智能回复子项 toggle 状态
  const [smartReplySubStates, setSmartReplySubStates] = useState<Record<string, boolean>>({
    commentReply: true,
    luckyDraw: true,
    greet: true,
    thankGift: true,
  ...
  const [activeBanboTab, setActiveBanboTab] = useState<BanboTab>('avatar')
  ...
  // ====== 伴播 Tab 内容渲染 ======
  const renderBanboTabContent = () => {
    switch (activeBanboTab) {
      case 'avatar':
        return (
  ...
  const renderConfigTabContent = () => {
    if (activeConfigTab === 'live') {
      return (
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 16px' }}>
          {/* Toggle 列表 */}
          {LIVE_TOGGLE_ITEMS.map(item => {
            const checked = liveToggleStates[item.id]
            return (
              <div key={item.id} style={{
                ...
                <Toggle checked={checked} onChange={(v) => {
                  setLiveToggleStates(prev => ({ ...prev, [item.id]: v }))
                  if (item.expandable && v) setSmartReplyExpanded(true)
                }} />
              </div>
            )
          })}
          ...
          {/* 智能回复展开态 */}
          {liveToggleStates.smartReply && smartReplyExpanded && (
            <div style={{
              marginTop: 12, padding: '12px 14px',
              background: '#F0F5FF', borderRadius: 8, border: `1px solid #D4E0FF`,
            }}>
              <div style={{
                ...
                <span style={{ fontSize: 14, fontWeight: 600, color: C.blue }}>智能回复</span>
                <div style={{ flex: 1, borderBottom: `2px solid ${C.border}`, marginLeft: 12 }} />
                <button onClick={() => setSmartReplyExpanded(false)} style={{
              ...
              <div style={{
                ...
                {SMART_REPLY_ITEMS.map(sub => {
                  const subChecked = smartReplySubStates[sub.id]
                  return (
                    <div key={sub.id} style={{
                      ...
                      <span style={{ fontSize: 13, color: C.text, textAlign: 'center' }}>{sub.label}</span>
                      <Toggle checked={subChecked} onChange={(v) => setSmartReplySubStates(prev => ({ ...prev, [sub.id]: v }))} />
                    </div>
                  )
                })}
              </div>
    ...
    return (
      <div style={{
        ...
        color: C.textTert, fontSize: 14, padding: '20px',
      }}>
        功能开发中...
      </div>
    )
  ...
  return (
    <div style={{
      position: 'absolute', top: 36, right: 0, bottom: 0, width: 380,
      ...
      <div style={{
        ...
        <span style={{ fontSize: 16, fontWeight: 600, color: C.text }}>助播虾配置面板</span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button style={{
            padding: '4px 12px', borderRadius: 6,
            border: `1px solid ${C.border}`, background: '#fff',
            color: C.textSec, fontSize: 12, cursor: 'pointer', fontFamily: f,
          }}>配置</button>
      ...
      <div style={{
        ...
        <button style={{
          padding: '6px 14px', borderRadius: 6,
          border: `1px solid ${C.border}`, background: '#fff',
          color: C.text, fontSize: 13, cursor: 'pointer', fontFamily: f,
        }}>形象</button>
        <button style={{
          padding: '6px 14px', borderRadius: 6,
          border: `1px solid ${C.border}`, background: '#fff',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 16 }}>🐟</span>
          <span style={{ fontSize: 13, color: C.text }}>伴播</span>
      ...
      {/* ===== 8 Tab 行 ===== */}
      <div style={{
        padding: '8px 16px', background: '#F7F8FA', borderBottom: `1px solid ${C.border}`,
        display: 'flex', gap: 4, overflowX: 'auto', flexShrink: 0,
      }}>
        {CONFIG_TABS.map(tab => (
          <button key={tab.key} onClick={() => setActiveConfigTab(tab.key)} style={{
            padding: '6px 12px', borderRadius: 6, border: 'none', fontSize: 13, whiteSpace: 'nowrap',
            fontWeight: activeConfigTab === tab.key ? 600 : 400,
            background: activeConfigTab === tab.key ? C.blueLight : 'transparent',
            color: activeConfigTab === tab.key ? C.blue : C.textSec,
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', position: 'relative' }}>
        ...
        <div style={{
          ...
          transition: 'width 0.3s', width: banboEnabled ? '50%' : '100%',
        }}>
        ...
        {banboEnabled && (
          <div style={{
            width: '50%', borderLeft: `1px solid ${C.border}`,
            background: '#fff', display: 'flex', flexDirection: 'column',
            overflow: 'hidden', flexShrink: 0,
          }}>
            {/* 伴播 Tab Bar */}
            <div style={{
              padding: '8px 12px', background: C.card, borderBottom: `1px solid ${C.border}`,
              display: 'flex', gap: 4, overflowX: 'auto', flexShrink: 0,
            }}>
              {BANBO_TABS.map(tab => (
                <button key={tab.key} onClick={() => setActiveBanboTab(tab.key)} style={{
                  padding: '6px 10px', borderRadius: 6, border: 'none', fontSize: 12, whiteSpace: 'nowrap',
                  fontWeight: activeBanboTab === tab.key ? 600 : 400,
                  background: activeBanboTab === tab.key ? C.blueLight : 'transparent',
                  color: activeBanboTab === tab.key ? C.blue : C.textSec,
                  cursor: 'pointer', fontFamily: f, transition: 'all 0.15s',
                }}>{tab.label}</button>
              ))}
            </div>
      ...
      <div style={{
        ...
        textAlign: 'center', color: C.textTert, fontSize: 12,
