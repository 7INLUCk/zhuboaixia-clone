...
export default function VoiceSwitchPanel({ onClose, onSwitchPanel }: Props) {
  ...
  const [customScript, setCustomScript] = useState('来，我们一起看看{商品名}，今天特惠价只要{价格}！')
  // 伴播造型切换
  const [outfitSwitch, setOutfitSwitch] = useState(false)
  // 商品-造型绑定表：productId → outfitId（null = 不切换）
  const [bindings, setBindings] = useState<Record<string, string | null>>({
    p1: 'o1', p2: null, p3: 'o2', p4: null,
  })
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[
            ...
            { key: 'sceneLayout' as const, label: '🎬 画面布局' },          ].map(tab => (
            <button key={tab.key} onClick={() => onSwitchPanel?.(tab.key)} style={{
              ...
              fontWeight: tab.key === 'voiceSwitch' ? 600 : 400,
              background: tab.key === 'voiceSwitch' ? C.blueLight : 'transparent',
              color: tab.key === 'voiceSwitch' ? C.blue : C.textSec,
          ...
          <>
                  ...
                  <div style={{ marginTop: 10 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      ...
                      <div style={{
                        ...
                        <span style={{ width: 120, textAlign: 'center' }}>切换造型</span>
                      </div>

                      {LIVE_PRODUCTS.map(prod => {
                        const boundOutfit = bindings[prod.id] ?? null
                        return (
                          <div key={prod.id} style={{
                            display: 'flex', alignItems: 'center', gap: 8,
                            padding: '8px 10px', borderRadius: 8,
                            background: boundOutfit ? '#FFFBF0' : '#FAFAFA',
                            border: boundOutfit ? '1px solid #FFE58F' : `1px solid ${C.border}`,
                          }}>
                            {/* 商品信息 */}
                            ...
                            <select
                              value={boundOutfit ?? ''}
                              onChange={e => setBindings(prev => ({
                                ...prev,
                                [prod.id]: e.target.value || null,
                              }))}
                              onClick={e => e.stopPropagation()}
                              style={{
                                width: 110, height: 28, borderRadius: 6,
                ...
                {ttsEnabled && (
                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 11, color: C.textSec, marginBottom: 6 }}>
                      话术模板（变量：<code style={{ background: '#f0f0f0', padding: '1px 4px', borderRadius: 3 }}>{'{商品名}'}</code> <code style={{ background: '#f0f0f0', padding: '1px 4px', borderRadius: 3 }}>{'{价格}'}</code> <code style={{ background: '#f0f0f0', padding: '1px 4px', borderRadius: 3 }}>{'{链接号}'}</code>）
                    </div>
                    <textarea value={customScript} onChange={e => setCustomScript(e.target.value)} style={{
                      width: '100%', height: 64, padding: '10px 12px', borderRadius: 8,
                      border: `1px solid ${C.border}`, fontSize: 13, fontFamily: C.font,
                      resize: 'none', outline: 'none', boxSizing: 'border-box', lineHeight: 1.5,
                    }} />
                    <div style={{ marginTop: 6, fontSize: 11, color: C.textTert }}>
                      预览：<span style={{ color: C.textSec }}>
                        {customScript.replace('{商品名}', '助播虾落地手机直播支架').replace('{价格}', '¥999').replace('{链接号}', '1')}
                      </span>
                    </div>
                  </div>
                )}
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: C.textSec, flexWrap: 'wrap' }}>
                ...
                {ttsEnabled && (<>
                  <span style={{ color: C.textTert }}>+</span>
                  <span style={{ padding: '4px 8px', background: '#E8F3FF', borderRadius: 6, border: `1px solid #B8D4FF`, color: C.blue }}>🔊 伴播出声</span>
                </>)}
              </div>
            </div>
          </>
